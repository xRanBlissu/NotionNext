...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    const excludeProperties = ['date', 'select', 'multi_select', 'person', 'checkbox']
    const properties = {}
    for (let i = 0; i < rawProperties.length; i++) {
      ...
      if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
        ...
        switch (schema[key]?.type) {
          case 'checkbox': {
            // 处理复选框字段 - Notion的复选框存储为 [['Yes']] 或 undefined
            properties[schema[key].name] = val?.[0]?.[0] === 'Yes'
            break
          }
          case 'date': {
    ...
    if (properties) {
      // 将表格中的字段映射成 英文
      const config = {
        enable: (properties['启用'] || properties.Enable) === true || (properties['启用'] || properties.Enable) === 'Yes',
