      ...
      notionAPI = {
        ...
        getPage: async (pageId) => {
          try {
            ...
            // 如果是数据库页面，查询数据库内容
            if (pageRecord.parent && (pageRecord.parent.type === 'database_id' || pageRecord.object === 'database')) {
              try {
                ...
                // 如果数据库查询失败，可能pageId实际上是页面ID，尝试获取页面内容
                console.log('数据库查询失败，尝试作为页面处理:', dbError.message)
              }
            }
