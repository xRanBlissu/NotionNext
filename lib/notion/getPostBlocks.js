...
function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    // 修复 undefined 值导致的序列化错误
    if (b?.value) {
      // 修复顶层字段的 undefined 值
      if (b.value.cover === undefined) {
        b.value.cover = null
      }
      if (b.value.icon === undefined) {
        b.value.icon = null
      }
      if (b.value.file_ids === undefined) {
        b.value.file_ids = null
      }
      
      // 修复 format 对象中的 undefined 值
      if (b.value.format) {
        if (b.value.format.page_cover === undefined) {
          b.value.format.page_cover = null
        }
        if (b.value.format.page_icon === undefined) {
          b.value.format.page_icon = null
        }
        // 递归处理 format 对象中的所有字段
        Object.keys(b.value.format).forEach(key => {
          if (b.value.format[key] === undefined) {
            b.value.format[key] = null
          }
        })
      }
      
      // 递归处理 properties 中的 undefined 值
      if (b.value.properties) {
        Object.keys(b.value.properties).forEach(key => {
          if (b.value.properties[key] === undefined) {
            b.value.properties[key] = null
          }
        })
      }
    }
